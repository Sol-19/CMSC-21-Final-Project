...
void setBoard(Piece board [8][8])
{
    ...
    board[1][0].type = KING;  board[1][0].color = WHITE;  // e1 - White King
    board[1][1].type = PAWN;  board[1][1].color = WHITE;  // e7 - Pawn (pinned)

    // Black
    board[4][0].type = KING;  board[4][0].color = BLACK;  // e8 - Black King
    board[1][3].type = ROOK;  board[1][3].color = BLACK;  // e5 - Black Rook pinning the pawn
